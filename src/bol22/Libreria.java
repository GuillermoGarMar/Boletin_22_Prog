...
public class Libreria {
    ...
    public void engadir() {
        libreria.add(new Libro(dameTitulo(), dameAutor(), (int) pedirPrecio(), pedirNumExemplares()));
    }
    ...
    public void consultar() {
        ...
        for (int i = 0; i < libreria.size(); i++) {
            if (libreria.get(i).Nome.equals(libros)) {
                System.out.println("O libro está na lista");
                atopado = true;
                System.out.println("Titulo do libro: " + libreria.get(i).Nome + " Precio: " + (libreria.get(i).precio));
            }
        }
        if (atopado == false) {
    ...
    public void consultarAutor() {
        ...
        for (int i = 0; i < libreria.size(); i++) {
            if (libreria.get(i).autor.equals(libros)) {
                ...
                atopado = true;
                System.out.println(libreria.get(i).autor + " titulo:" + libreria.get(i).Nome);
            }
        }
        if (atopado == false) {
            System.out.println("O libro non está na lista");
        ...
        do {
            ...
            if (s1 != null) {
                ...
                while (st.hasMoreTokens()) {
                    libreria.add(new Libro(st.nextToken(), st.nextToken(), (int) Float.parseFloat(st.nextToken()), Integer.parseInt(st.nextToken())));
    ...
    public void modificar() {
        ...
        for (int i = 0; i < libreria.size(); i++) {
            if (libreria.get(i).getNome().equals(modificar)) {
                libreria.get(i).setAutor(dameAutor());
                libreria.get(i).setNome(dameTitulo());
